...
- [x] Create `tenant_memberships` table
- [x] Add `tenant_id` to all core tables
- [x] Create indexes on `tenant_id` columns
- [x] Enable RLS on all tenant tables
- [x] Create RLS policies for each table
- [x] Create `app_user` and `migration_user` roles
- [x] Add tenant context middleware to FastAPI
- [x] Bootstrap default tenant on install
- [x] Create `qr_namespaces` table
- [x] Create `service_accounts` and `service_tokens` tables

### Tests (MVP)

- [x] Test: Two tenants can have products with same name
- [x] Test: Queries without tenant context fail
- [x] Test: Token in tenant A cannot resolve tenant B objects
- [x] Test: Service token is restricted to its tenant
- [x] Test: RLS prevents cross-tenant data access

...
# 15. Frontend & PWA Architecture

### DEC-051: UI stack and PWA alignment
- **Decision:** Vue 3 + Quasar PWA is the **target** architecture. The current NiceGUI implementation is **interim** for rapid prototyping. Migration to Vue/Quasar + full PWA capabilities is planned for Phase 3.
- **Status:** Approved
- **Rationale:** PRDs describe a comprehensive Vue/Quasar PWA architecture with offline support, service workers, and installability. The current codebase uses NiceGUI (Python, server-rendered via WebSocket) served by FastAPI. Keeping Vue/Quasar as the target preserves the architectural vision while acknowledging NiceGUI as a functional interim solution.
- **Phase:**
  - **[CORE]** NiceGUI provides working UI for Phases 1-2 (auth, inventory, scanning).
  - **[CORE]** Phase 3 migrates to Vue 3 + Quasar + Vite + Pinia + Vue Router.
  - **[CORE]** Phase 3 includes PWA manifest, service worker (vite-plugin-pwa/Workbox), offline support.
  - **[FLEX]** Capacitor for native iOS/Android app-store builds if needed.
- **Notes:**
  - Current deployment: Single FastAPI process serving NiceGUI on port 3334.
  - Target deployment: Separate Vue frontend (port 3335) + FastAPI backend (port 3334).
  - PRD sections 30.10, 30.13, 40.15, 80.13 describe the target architecture with "Current Implementation" notes for NiceGUI.
...
- DEC-051: UI stack (Vue/Quasar target, NiceGUI interim)

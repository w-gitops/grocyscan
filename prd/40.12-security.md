...
### 12.4.1 HTTPS Requirement (Production)

**HTTPS is required in production** for the following reasons:

- **PWA installability**: Service workers require HTTPS (except localhost for development).
- **Secure cookies**: Session cookies are configured with `secure=True`.
- **Browser security features**: Modern security APIs require secure contexts.
- **Data protection**: Encrypt data in transit to protect credentials and sensitive inventory data.

**Implementation:**

- Configure reverse proxy (Caddy recommended) to terminate TLS.
- **Redirect HTTP to HTTPS**: All HTTP requests must redirect to HTTPS (301 permanent redirect).
- Use valid certificates (Let's Encrypt recommended for self-hosted).
...
**Development exception:** `localhost` and `127.0.0.1` do not require HTTPS for service worker and PWA testing during development.

### 12.4.2 Security Headers (Future)

For enhanced security, consider these headers (marked as **[FLEX]**):

| Header | Purpose |
|--------|---------|
| `Cross-Origin-Opener-Policy (COOP)` | Isolate browsing context |
| `Cross-Origin-Embedder-Policy (COEP)` | Control cross-origin resource loading |
| `Strict-Transport-Security (HSTS)` | Enforce HTTPS |
| `Permissions-Policy` | Control browser features |

These may be required if Homebot integrates SharedArrayBuffer or other advanced APIs in the future.

...
- **Same fingerprint**: The localStorage UUID remains the same whether the app is accessed via browser or installed PWA. This means the device record is shared—which is typically the desired behavior.
...
|------|---------------|----------|
| **Add Stock** | Scan → add 1 unit to default location | Unpacking groceries |
| **Consume** | Scan → remove 1 unit (FIFO) | Taking item from pantry |
| **Transfer** | Scan → prompt for destination | Moving items |
| **Lookup** | Scan → show product info only | Checking expiration |
| **Inventory** | Scan → show stock levels, edit | Stock taking |
...
│                                         │
├─────────────────────────────────────────┤
│  Default Location: Kitchen Pantry       │
│  [Change]                               │
└─────────────────────────────────────────┘
```

...
## 8.5 Docker Compose Observability Stack
...
Key panels for the GrocyScan Overview dashboard:

| Panel | Type | Query |
|-------|------|-------|
| Scans per Minute | Timeseries | `rate(grocyscan_scans_total[1m])` |
| Lookup Latency (P95) | Timeseries | `histogram_quantile(0.95, rate(grocyscan_lookup_duration_seconds_bucket[5m]))` |
| Cache Hit Rate | Gauge | `sum(rate(grocyscan_cache_operations_total{operation="hit"}[5m])) / sum(rate(grocyscan_cache_operations_total[5m]))` |
| Job Queue Status | Stat | `grocyscan_job_queue_size` |
| Error Rate | Timeseries | `rate(http_requests_total{status=~"5.."}[5m])` |
| Recent Logs | Logs | `{service_name="grocyscan"}` |

---

## Navigation

- **Previous:** [UI Specification](07-ui-specification.md)
- **Next:** [Installation & Operations](09-installation-operations.md)

...
spinner() {
  ...
  local i=0
  while true; do
    printf "\r  ðŸ› Agent working... %s  (watch: tail -f %s/.ralph/activity.log)" "${spin:i++%${#spin}:1}" "$workspace" >&2
    sleep 0.1
  done
}
...
run_iteration() {
  ...
  echo "ðŸ› Ralph Iteration $iteration" >&2
  echo "â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•" >&2
  echo "" >&2
  ...
  while IFS= read -r line; do
    case "$line" in
      "ROTATE")
        ...
        echo "ðŸ”„ Context rotation triggered - stopping agent..." >&2
      ...
      "GUTTER")
        printf "\r\033[K" >&2  # Clear spinner line
        echo "ðŸš¨ Gutter detected - agent may be stuck..." >&2
        signal="GUTTER"
...
run_ralph_loop() {
  ...
  if [[ -n "$USE_BRANCH" ]]; then
    echo "ðŸŒ¿ Creating branch: $USE_BRANCH"
    ...
    case "$signal" in
      "COMPLETE")
        ...
        if [[ "$task_status" == "COMPLETE" ]]; then
          ...
          echo ""
          echo "â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•"
          echo "ðŸŽ‰ RALPH COMPLETE! Agent signaled completion and all criteria verified."
          echo "â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•"
          echo ""
          echo "Completed in $iteration iteration(s)."
          echo "Check git log for detailed history."
        ...
        ;;
      "ROTATE")
        log_progress "$workspace" "**Session $iteration ended** - ðŸ”„ Context rotation (token limit reached)"
        echo ""
        ...
        ;;
      "GUTTER")
        log_progress "$workspace" "**Session $iteration ended** - ðŸš¨ GUTTER (agent stuck)"
        echo ""
        echo "ðŸš¨ Gutter detected. Check .ralph/errors.log for details."
      ...
      *)
        ...
        if [[ "$task_status" == INCOMPLETE:* ]]; then
          ...
          echo ""
          echo "ðŸ“‹ Agent finished but $remaining_count criteria remaining."
...
check_prerequisites() {
  ...
  # Check for task file
  if [[ ! -f "$task_file" ]]; then
    echo "âŒ No RALPH_TASK.md found in $workspace"
    echo ""
  ...
  if ! command -v cursor-agent &> /dev/null; then
    echo "âŒ cursor-agent CLI not found"
    echo ""
    echo "Install via:"
...
show_banner() {
  echo "â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•"
  echo "ðŸ› Ralph Wiggum: Autonomous Development Loop"
  echo "â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•"
  echo ""
